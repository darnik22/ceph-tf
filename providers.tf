resource "openstack_networking_floatingip_v2" "providers" {
  depends_on = ["openstack_compute_instance_v2.providers"]
  port_id  = "${element(openstack_networking_port_v2.providers-port.*.id, count.index)}"
  count = "${var.provider_count}"
  pool  = "${var.external_network}"
}

resource "null_resource" "provision-provider" {
  count = "${var.provider_count}"
  depends_on = ["openstack_networking_floatingip_v2.providers"]
  connection {
    host     = "${element(openstack_networking_floatingip_v2.providers.*.address, count.index)}"
...
resource "openstack_compute_instance_v2" "providers" {
  ...
  network {
    port = "${element(openstack_networking_port_v2.providers-port.*.id, count.index)}"
    uuid = "${openstack_networking_network_v2.network.id}"
    access_network = true
  }
}

resource "openstack_networking_port_v2" "providers-port" {
  count              = "${var.provider_count}"
